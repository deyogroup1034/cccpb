...
function HomePage({ go }) {
  const { event, scriptures, whatWeDo, history, mission, whoComes } = window.CCCPB;
  ...
  return (
    <div className="page" data-screen-label="home page">
      ...
      <section className="hero" data-screen-label="hero">
        <div className="container">
          <div className="hero__grid">
            <div>
              <div className="ndop-badge" style={{ marginBottom: 18 }}><i></i>National Day of Prayer · May 6, 2027</div>
              <div className="hero__date">{event.edition} Collin County Christian Prayer Breakfast</div>
              <h1 className="hero__title">
                Great leaders<br/>
                <em>kneel first.</em>
              </h1>
              <p className="hero__sub">
                One morning. One county, at one table. Churches, city halls, schools, first responders, businesses, and families gather over breakfast on the National Day of Prayer — with a keynote from Texas Rangers Hall of Famer <b>Elvis Andrus</b>.
              </p>
      ...
      {/* WHO COMES */}
      <section className="section" data-screen-label="who comes">
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 20, marginBottom: 44 }}>
            <div>
              <div className="eyebrow">Everyone is welcome</div>
              <h2 style={{ marginTop: 14 }}>Who's in the room.</h2>
            </div>
            <p style={{ fontSize: 16, color: 'var(--ink-2)', maxWidth: 420, lineHeight: 1.65 }}>
              This is a prayer breakfast before it is anything else. You don't need a company, a title, or an invitation to take a seat.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: 'var(--rule-soft)', border: '1px solid var(--rule-soft)' }} className="pillars-grid">
            {whoComes.map((w, i) => (
              <div key={i} style={{ background: 'var(--bone)', padding: '32px 26px' }}>
                <div className="flagrule" style={{ width: 40, marginBottom: 18 }}></div>
                <h3 style={{ fontSize: 22, lineHeight: 1.2 }}>{w.t}</h3>
                <p style={{ marginTop: 10, fontSize: 14.5, color: 'var(--ink-2)', lineHeight: 1.65 }}>{w.b}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section" data-screen-label="speaker preview">
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: 64, alignItems: 'center' }} className="speaker-prev-grid">
            ...
            <div>
              <div className="eyebrow">2027 Keynote Speaker</div>
      ...
      {/* GET INVOLVED band */}
      <section className="section" style={{ background: 'var(--navy)', color: '#fff' }} data-screen-label="get involved band">
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: 64, alignItems: 'center' }} className="about-grid">
            <div>
              <div className="eyebrow" style={{ color: '#fff' }}>We need help</div>
              <h2 style={{ marginTop: 16, color: '#fff' }}>Sponsors, in‑kind partners, and <em style={{ color: 'var(--navy-soft)' }}>fifty volunteers.</em></h2>
              <p style={{ marginTop: 18, fontSize: 16.5, opacity: 0.82, lineHeight: 1.7, maxWidth: 420 }}>
                No paid staff, 600 seats to fill, and one morning to get it right. Sponsorships underwrite the breakfast so every ticket dollar can go back into Collin County.
              </p>
              <div style={{ marginTop: 30, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <button className="btn btn--oxblood" onClick={() => go('involved')}>Ways to help</button>
                <button className="btn btn--ghost" style={{ color: '#fff', borderColor: 'rgba(255,255,255,0.5)' }} onClick={() => go('tickets')}>Sponsorship levels →</button>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="involved-grid">
              {[
                { v: '18', k: 'Sponsorships open' },
                { v: '6', k: 'In‑kind needs' },
                { v: '50', k: 'Volunteers needed' },
              ].map(s => (
                <div key={s.k} style={{ border: '1px solid rgba(255,255,255,0.18)', padding: '28px 22px' }}>
                  <div style={{ fontFamily: 'var(--serif)', fontSize: 52, fontWeight: 600, lineHeight: 1 }}>{s.v}</div>
                  <div style={{ marginTop: 10, fontSize: 10.5, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 700, color: 'var(--navy-soft)' }}>{s.k}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, border: '1px solid var(--ink)' }} className="cta-grid">
            <div style={{ padding: '64px 56px', background: 'var(--ink)', color: 'var(--cream)' }}>
              ...
              <p style={{ marginTop: 22, opacity: 0.75, fontSize: 16, lineHeight: 1.65, maxWidth: 380 }}>
                Individual tickets and full tables are open now. Sponsorships are limited and close on April 1, 2027.
              </p>
  ...
  return (
    <div className="page" data-screen-label="about page">
      <section className="section">
        <div className="container container--narrow" style={{ textAlign: 'center' }}>
          ...
          <h1 style={{ marginTop: 22, fontSize: 'clamp(48px, 5.4vw, 80px)' }}>
            A county at <em style={{ color: 'var(--flag-red)' }}>prayer</em><br/> since 1999.
          </h1>
          <p style={{ marginTop: 28, fontSize: 19, color: 'var(--ink-2)', lineHeight: 1.65, maxWidth: 640, marginLeft: 'auto', marginRight: 'auto' }}>
            What began as a handful of neighbors around a single breakfast table is now the morning Collin County sets aside, every National Day of Prayer, to pray together.
          </p>
        </div>
      </section>
      ...
      <section className="section" style={{ background: 'var(--cream-2)', textAlign: 'center' }}>
        <div className="container container--narrow">
          <h2>Will you join us in <em style={{ color: 'var(--flag-red)' }}>2027</em>?</h2>
      ...
      <section className="section">
        <div className="container container--narrow">
          ...
          <h1 style={{ marginTop: 22 }}>
            May 6, 2027.<br/>
            <em style={{ color: 'var(--flag-red)' }}>One quiet morning.</em>
          </h1>
          ...
          <div style={{ marginTop: 24, padding: 24, background: 'var(--bone)', border: '1px solid var(--rule-soft)' }}>
            <div style={{ display: 'flex', gap: 32, alignItems: 'baseline', flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontFamily: 'var(--serif)', fontSize: 26, fontWeight: 500 }}>May 6, 2027</div>
              </div>
              <div>
